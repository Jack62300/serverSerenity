...
function OpenPersonnelMenu()
	...
	ESX.TriggerServerCallback('NB:getUsergroup', function(group)
		...
		table.insert(elements, {label = 'Animations',					value = 'menuperso_actions2'})
		if (IsInVehicle()) then
		...
		if playergroup == 'mod' or playergroup == 'admin' or playergroup == 'superadmin' or playergroup == 'owner' then
			...
			--table.insert(elements, {label = 'Panel Modération',				value = 'menuperso_modo2'})
		end
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if playergroup == 'superadmin' or playergroup == 'owner' then
					...
					--table.insert(elements, {label = 'Changer l\'apparence',									value = 'menuperso_modo_changer_skin'})
					--table.insert(elements, {label = 'Sauvegarder l\'apparence',									value = 'menuperso_modo_save_skin'})
					table.insert(elements, {label = 'ouvrir vehicule',									value = 'menuperso_modo_carjack'})
					table.insert(elements, {label = 'menotter',									value = 'menuperso_modo_menottes'})
					table.insert(elements, {label = 'Escorter',									value = 'menuperso_modo_drag'})
				end

				if data.current.value == 'menuperso_modo2' then
					openAdminMenu2()
				end
				...
				if data.current.value == 'menuperso_modo' then
					ESX.UI.Menu.Open(
						...
						function(data2, menu2)
							...
							if data2.current.value == 'menuperso_modo_carjack' then
							...
							if data2.current.value == 'menuperso_modo_menottes' then
							local closestPlayer = ESX.Game.GetClosestPlayer()
							TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(closestPlayer))
							end
							if data2.current.value == 'menuperso_modo_drag' then
							local closestPlayer = ESX.Game.GetClosestPlayer()
				...
				if data.current.value == 'menuperso_actions2' then
					openMenuAction()
				end
				...
				if data.current.value == 'menuperso_moi' then
					...
					table.insert(elements, {label = 'Voir ma carte d\'identitée',							value = 'menuperso_moi_id'})
					table.insert(elements, {label = 'Voir mes permis',							value = 'checkDriver'})
					table.insert(elements, {label = 'Montrer ma carte d\'identitée',							value = 'showID'})
					table.insert(elements, {label = 'Montrer mes permis',							value = 'showDriver'})
					table.insert(elements, {label = 'Mon animal',    							value = 'menuperso_moi_animal'})
					...
					table.insert(elements, {label = 'Derober la Personne',    							value = 'menuperso_moi_vol'})
					table.insert(elements, {label = 'Masquer HUD',    							value = 'menuperso_moi_hud'})
					ESX.UI.Menu.Open(
						...
						function(data2, menu2)
							...
							if data2.current.value == 'menuperso_moi_id' then
							...
							if data2.current.value == 'menuperso_moi_hud' then
								func_showhud()
							end
							...
							if data2.current.value == 'menuperso_moi_vol' then
								openVol()
							end
							...
							if distance ~= -1 and distance <= 3.0 then
								...
								if data2.current.value == 'showDriver' then
...
function func_showhud()
    if showhud then
        ...
        TriggerEvent('street:Enable_compass')
        TriggerEvent('street:Enable_streetName')
        TriggerEvent('esx_status:Enable')
        TriggerEvent('initialspeedo:Enable')
        TriggerEvent('advancedFuel:Enable')
        TriggerEvent('carhud:Enable')
        TriggerEvent('time:Enable')
        DisplayRadar(true)
    else
        showhud = true
        TriggerEvent('ui:toggle', false)
        TriggerEvent('street:Disable_compass')
        TriggerEvent('street:Disable_streetName')
        TriggerEvent('esx_status:Disable')
        TriggerEvent('initialspeedo:Disable')
        TriggerEvent('advancedFuel:Disable')
        TriggerEvent('carhud:Disable')
        TriggerEvent('time:Disable')
        DisplayRadar(false)
    end
end
---------------------------------------------------------------------------Vehicule Menu
...
function openAdminMenu2()
	TriggerEvent('NB:closeAllSubMenu')
	TriggerEvent('NB:closeAllMenu')
	TriggerEvent('NB:closeMenuKey')
	
	TriggerEvent('NB:openMenuAdmin2')
end
