...
AddEventHandler('esx:playerLoaded', function(xPlayer)
 ...
 --[[ local orgTpl = '<div>{{org_label}} - {{gradeorg_label}}</div>' 
  ...
  local moneyTpl = '<div><img src="img/accounts/cash.png"/>{{money}}</div>' ---ici

  ESX.UI.HUD.RegisterElement('money', 1, 0, moneyTpl, {   --ici
  money = 0
  })

  ESX.UI.HUD.UpdateElement('money', {   --ici
  money = xPlayer.money
  }) 
...
AddEventHandler('es:activateMoney', function(money)
    ...
    ESX.UI.HUD.UpdateElement('money', {  ----ajoute ceci 
       money = money                 ------------ceci
    })                       ---------ceci
end)
...
AddEventHandler('esx:setOrg', function(org)
  ESX.UI.HUD.UpdateElement('org', {
    org_label   = org.label,
    gradeorg_label = org.grade_label
  })
end)
