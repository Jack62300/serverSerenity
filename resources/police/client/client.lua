...
local armoryStation = {
	{x=452.119966796875, y=-983.061966796875, z=30.690966796875} -- Mission Row
}
...
AddEventHandler('police:receiveIsCop', function(result)
	if(result == -1) then
		...
		if(rank >= config.rank.min_rank_set_rank) then
			TriggerEvent('chat:addSuggestion', "/copadd", "Donne l'accès au gps d'équipe", {{name = "id", help = "The ID of the player"}})
			TriggerEvent('chat:addSuggestion', "/coprem", "Retire l'accès au gps d'équipe", {{name = "id", help = "The ID of the player"}})
			TriggerEvent('chat:addSuggestion', "/coprank", "Ne pas utiliser", {{name = "id", help = "The ID of the player"}, {name = "rank", help = "The numeric value of the rank"}})
		else
