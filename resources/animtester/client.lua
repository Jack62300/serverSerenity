...
function DrawMainMenu()
    ...
    mainMenu.OnListChange = function(_, list, newindex)
        if list == AnimsDictList then
            --Utils.ShowNotification(AnimsDict[newindex])
...
function AddMenuName()
    local libNotif, animNotif, durNotif
    AnimDictMenu:Clear()
    if AnimsName[DictSelected] == nil then
        ...
        for k,v in pairs(AnimsName[DictSelected]) do
            ...
            --Item:RightLabel(Config.Vehicles[iresults[i]].price.."$", Colours.Green , Colours.Black)
    ...
    while true do Wait(0)
        ...
        if IsControlJustPressed(1, Config.OpenMenu) then --166=F5
            if mainMenu:Visible() then 
                ...
                --while not HasStreamedTextureDictLoaded("NativeUI") do
                    --Wait(100)
                --end
                _menuPool:CloseAllMenus()
                --DrawMainMenu()
