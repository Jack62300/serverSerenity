...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('bahamas_clothes_civil'), value = 'citizen_wear'},
				{label = _U('bahamas_clothes_bahamas'), value = 'bahamas_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'bahamas_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'mule3'},
		}
...
function OpenMobileBahamasActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_bahamas_actions',
		{
			title    = 'bahamas',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_bahamas', 'Bahamas', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_bahamasjob:hasEnteredMarker', function(zone)
	if zone == 'BouteilleFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas'  then
		CurrentAction     = 'bouteille_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementBouteille' and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas'  then
		CurrentAction     = 'bouteille_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementAlcool' and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas'  then
		CurrentAction     = 'Alcool_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'BahamasActions' and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' then
		CurrentAction     = 'bahamas_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' then
...
AddEventHandler('esx_bahamasjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'BouteilleFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' then
		TriggerServerEvent('esx_bahamasjob:stopHarvest')
	end  
	if (zone == 'TraitementBouteille' or zone == 'TraitementAlcool') and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' then
		TriggerServerEvent('esx_bahamasjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' then
		TriggerServerEvent('esx_bahamasjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.BahamasActions.Pos.x, Config.Zones.BahamasActions.Pos.y, Config.Zones.BahamasActions.Pos.z)
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' then
		...
		for k,v in pairs(Config.Zones)do
			if v.Type == 1 then
				...
				SetBlipSprite (blip2, 136)
				SetBlipDisplay(blip2, 4)
				SetBlipScale  (blip2, 1.5)
				SetBlipColour (blip2, 8)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'bouteille_harvest' then
					TriggerServerEvent('esx_bahamasjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'bouteille_traitement' then
					TriggerServerEvent('esx_bahamasjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'alcool_traitement' then
					TriggerServerEvent('esx_bahamasjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_bahamasjob:startSell', CurrentActionData.zone)
				end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'bahamas' and (GetGameTimer() - GUI.Time) > 150 then
