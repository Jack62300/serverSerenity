...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('unicorn_clothes_civil'), value = 'citizen_wear'},
				{label = _U('unicorn_clothes_unicorn'), value = 'unicorn_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'unicorn_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
...
AddEventHandler('esx_unicornjob:hasEnteredMarker', function(zone)
	if zone == 'BouteilleFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn'  then
		CurrentAction     = 'bouteille_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementBouteille' and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn'  then
		CurrentAction     = 'bouteille_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementAlcool' and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn'  then
	...
	if zone == 'UnicornActions' and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn' then
		CurrentAction     = 'unicorn_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn' then
...
AddEventHandler('esx_unicornjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'BouteilleFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn' then
		TriggerServerEvent('esx_unicornjob:stopHarvest')
	end  
	if (zone == 'TraitementBouteille' or zone == 'TraitementAlcool') and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn' then
		TriggerServerEvent('esx_unicornjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'bouteille_harvest' then
					TriggerServerEvent('esx_unicornjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'bouteille_traitement' then
					TriggerServerEvent('esx_unicornjob:startTransform', CurrentActionData.zone)
				end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'unicorn' and (GetGameTimer() - GUI.Time) > 150 then
