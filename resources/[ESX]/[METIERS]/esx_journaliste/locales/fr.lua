Locales['fr'] = {
	-- Cloakroom
	['cloakroom']				 = 'Vestiaire',
	['citizen_wear']			 = 'Tenue civile',
	['journaliste_outfit']		 = 'Tenue de stagiaire',
	['journaliste_outfit_1']	 = 'Tenue de reporter',
	['journaliste_outfit_2']	 = 'Tenue d\'enquêteur',
	['journaliste_outfit_3']	 = 'Tenue de directeur', 
    ['no_outfit']				 = 'Il n\'y a pas d\'uniforme à votre taille...',
    ['open_cloackroom']			 = 'Appuyez sur ~INPUT_CONTEXT~ pour vous changer',
	
	-- Vehicles  
    ['vehicle_menu']             = 'Véhicule',
    ['vehicle_out']              = 'Il y a déja un véhicule dehors',
    ['vehicle_spawner']          = 'Appuyez sur ~INPUT_CONTEXT~ pour sortir un véhicule',
    ['store_vehicle']            = 'Appuyez sur ~INPUT_CONTEXT~ pour ranger le véhicule',
    ['service_max']              = 'Service complet : ',
    ['spawn_point_busy']         = 'Un véhicule occupe le point de sortie',
	
	-- Boss Menu
	['deposit_society'] 		 = 'déposer argent',
	['withdraw_society']		 = 'retirer argent société',
	['boss_actions']			 = 'action Patron',
	-- Misc
	['invalid_amount']			 = '~r~montant invalide',
	['open_menu']				 = 'appuyez sur ~INPUT_CONTEXT~ pour ouvrir le menu',
	['deposit_amount']			 = 'montant du dépôt',
	['money_withdraw']			 = 'montant du retrait',
 
    -- Vault  
    ['get_weapon']               = 'Prendre Arme',
    ['put_weapon']               = 'Déposer Arme',
    ['get_weapon_menu']          = 'Coffre - Prendre Arme',
    ['put_weapon_menu']          = 'Coffre - Déposer Arme',
    ['get_object']               = 'Prendre Objet',
    ['put_object']               = 'Déposer Objet',
    ['vault']                    = 'Coffre',
    ['open_vault']               = 'Appuyez sur ~INPUT_CONTEXT~ pour accéder au coffre',

    -- Boss Menu  
    ['take_company_money']       = 'Retirer argent société',
    ['deposit_money']            = 'Déposer argent',
    ['amount_of_withdrawal']     = 'Montant du retrait',
    ['invalid_amount']           = 'Montant invalide',
    ['amount_of_deposit']        = 'Montant du dépôt',
    ['open_bossmenu']            = 'Appuyez sur ~INPUT_CONTEXT~ pour ouvrir le menu',
    ['invalid_quantity']         = 'Quantité invalide',
    ['you_removed']              = 'Vous avez retiré x',
    ['you_added']                = 'Vous avez ajouté x',
    ['quantity']                 = 'Quantité',
    ['inventory']                = 'Inventaire',
    ['unicorn_stock']            = 'Stock',

    -- Misc 
    ['map_blip']                 = 'Weazel News',

    -- Billing Menu  
    ['billing']                  = 'Facture',
    ['no_players_nearby']        = 'Aucun joueur à proximité',
    ['billing_amount']           = 'Montant de la facture',
    ['amount_invalid']           = 'Montant invalide',

    -- Teleporters  
    ['from_boss_desk_to_in_msg'] = 'Appuyez sur ~INPUT_CONTEXT~ pour entrer dans le bureau de Mark',
    ['from_in_to_boss_desk_msg'] = 'Appuyez sur ~INPUT_CONTEXT~ pour sortir du bureau de mark',
    ['from_reunion_to_in_msg']   = 'Appuyez sur ~INPUT_CONTEXT~ pour entrer dans la salle de réunion',
    ['from_in_to_reunion_msg']   = 'Appuyez sur ~INPUT_CONTEXT~ pour sortir de la salle de réunion',
    ['from_roof_to_floor_msg']   = 'Appuyez sur ~INPUT_CONTEXT~ pour monter sur le toit',
    ['from_floor_to_roof_msg']   = 'Appuyez sur ~INPUT_CONTEXT~ pour descendre au rez-de-chaussée',
    ['from_roof_to_first_msg']   = 'Appuyez sur ~INPUT_CONTEXT~ pour monter sur le toit',
    ['from_first_to_roof_msg']   = 'Appuyez sur ~INPUT_CONTEXT~ pour descendre au premier',
    ['from_first_to_floor_msg']  = 'Appuyez sur ~INPUT_CONTEXT~ pour monter au premier étage',
    ['from_floor_to_first_msg']  = 'Appuyez sur ~INPUT_CONTEXT~ pour descendre au rez-de-chaussée',
    ['from_roof_to_lan_msg']     = 'Appuyez sur ~INPUT_CONTEXT~ pour sortir de la salle de LAN',
    ['from_lan_to_roof_msg']     = 'Appuyez sur ~INPUT_CONTEXT~ pour entrer dans la salle de LAN',
    
}