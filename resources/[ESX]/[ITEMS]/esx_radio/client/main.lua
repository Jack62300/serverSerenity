...
Citizen.CreateThread(function()
            while true do
                Citizen.Wait(300)
                    if IsControlPressed(2, Keys["LEFTALT"]) and Busy == false and PlayerData.job.name == "police" then
                    ESX.TriggerServerCallback('esx_walkie:getItemAmount', function(quantity)
                    if quantity > 0 then
                        TriggerServerEvent("esx_walkie:playSoundWithinDistanceServer", 10, "copradio", 0.1) 
                    ...
                    elseif not IsControlPressed(2, Keys["LEFTALT"]) and Busy == true and PlayerData.job.name == "police" then
                        -- Deaktiviere esx_walkie Talkie
                        TriggerServerEvent("esx_walkie:playSoundWithinDistanceServer", 10, "copradiooff", 0.1) 
