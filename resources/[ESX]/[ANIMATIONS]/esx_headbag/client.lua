...
if closestPlayer == -1 or closestDistance > 2.0 then 
  ...
  if not HaveBagOnHead then
    ...
    ESX.ShowNotification('~g~Vous avez mit un sac sur la tete de ~w~' .. GetPlayerName(closestPlayer))
...
AddEventHandler('esx_worek:zdejmijc', function(gracz)
    ESX.ShowNotification('~g~On vous a retire le sac sur votre tete!')
    DeleteEntity(Worek)
    SetEntityAsNoLongerNeeded(Worek)
    SendNUIMessage({type = 'closeAll'})
    HaveBagOnHead = false
...
function OpenBagMenu() --This function is menu function
    ...
    local elements = {
          {label = 'Mettre le sac sur la tete', value = 'puton'},
          {label = 'Enlever le sac de la tete', value = 'putoff'},
        ...
        elements = elements
            ...
            function(data2, menu2)
              ...
              if distance ~= -1 and distance <= 2.0 then
                ...
                ESX.ShowNotification('~r~Pas de joueur a proximite.')
              end
            end,
      function(data2, menu2)
        menu2.close()
