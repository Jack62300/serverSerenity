...
AddEventHandler('engineoff',function() 
        ...
        if (IsPedSittingInAnyVehicle(player)) then 
            local vehicle = GetVehiclePedIsIn(player,false)
			engineoff = true
			ShowNotification("Moteur ~r~off~s~.")
			while (engineoff) do
			SetVehicleEngineOn(vehicle,false,false,false)
			SetVehicleUndriveable(vehicle,true)
			Citizen.Wait(0)
...
AddEventHandler('engineon',function() 
    local player = GetPlayerPed(-1)
        ...
        if (IsPedSittingInAnyVehicle(player)) then 
			...
			SetVehicleUndriveable(vehicle,false)
			SetVehicleEngineOn(vehicle,true,false,false)
			ShowNotification("Moteur ~g~on~s~.")
	end
end)
-- T R U N K --
...
AddEventHandler('trunk',function() 
	local player = GetPlayerPed(-1)
			...
			if distanceToVeh <= interactionDistance then
				...
				ShowNotification("~r~Vous devez etre a coter de votre vehicule.")
			end
end)
-- R E A R  D O O R S --
	...
	local distanceToVeh = GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(vehicle), 1)
		if DoesEntityExist(vehicle) then
			if distanceToVeh <= lockDistance then
				...
				ShowNotification("Vous avez verrouiller votre ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~.")
				TriggerEvent('lockLights')
				else
				SetVehicleDoorsLocked(vehicle,1)
				ShowNotification("Vous avez deverrouiller votre ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~.")
				TriggerEvent('lockLights')
				end
			else
				ShowNotification("~r~Vous devez etre a coter de votre vehicule.")
			...
			ShowNotification("~r~Vehicule non sauvegarder")
		end
	end)
...
AddEventHandler('save',function() 
	...
	if (IsPedSittingInAnyVehicle(player)) then 
		if  saved == true then
			...
			ShowNotification("Vehicule ~r~non sauvegarder~w~.")
			saved = false
		else
			RemoveBlip(targetBlip)
			...
			ShowNotification("Cette ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle))) .. "~w~ est votre~g~ vehicule ~w~sauvegarder.")
			saved = true
		end
	end
end)
...
AddEventHandler('controlsave',function() 
		if controlsave_bool == false then
			...
			ShowNotification("Vous ne controler plus votre ~y~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(saveVehicle))))
			end
		else
