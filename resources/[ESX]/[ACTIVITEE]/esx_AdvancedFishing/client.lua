...
--local blip2 = AddBlipForCoord(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z)
			...
			--BeginTextCommandSetBlipName("STRING")
			--AddTextComponentString("Dealer de tortue")
...
--local blip3 = AddBlipForCoord(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z)
			...
			--BeginTextCommandSetBlipName("STRING")
			--AddTextComponentString("Dealer de requin")
	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				fishing = false
				ESX.ShowNotification("~r~Pêche fini")
			end
			if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~Le poisson s'est echapé")
				end
			end
		end
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("~g~le poisson a mordu \n ~h~Press " .. correct .. " pour l'attraper")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~y~Tu ne peux pas pecher depuis ce vehicule")
	else
		if pos.y >= 7700 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 then
			ESX.ShowNotification("~g~Que la pêche commence")
			TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_STAND_FISHING", 0, true)
			fishing = true
		else
			ESX.ShowNotification("~y~Il faut aller plus loin du port")
		end
	end
...
function OpenBoatsMenu(x, y , z)
	...
	local elements = {}
		...
		table.insert(elements, {label = '<span style="color:green;">Dinghy</span> <span style="color:red;">10$</span>', value = 'boat'})
		table.insert(elements, {label = '<span style="color:green;">Suntrap</span> <span style="color:red;">35$</span>', value = 'boat6'}) 
		table.insert(elements, {label = '<span style="color:green;">Jetmax</span> <span style="color:red;">45$</span>', value = 'boat5'}) 	
		table.insert(elements, {label = '<span style="color:green;">Toro</span> <span style="color:red;">55$</span>', value = 'boat2'}) 
		table.insert(elements, {label = '<span style="color:green;">Marquis</span> <span style="color:red;">30$</span>', value = 'boat3'}) 
		table.insert(elements, {label = '<span style="color:green;">Tug boat</span> <span style="color:red;">40$</span>', value = 'boat4'})
	...
	if data.current.value == 'boat' then
		...
		TriggerEvent("chatMessage", 'Tu as louer un bateau pour', {255,0,255}, '$' .. 10)
	...
	if data.current.value == 'boat2' then
		...
		TriggerEvent("chatMessage", 'Tu as louer un bateau pour', {255,0,255}, '$' .. 20)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat3' then
		...
		TriggerEvent("chatMessage", 'Tu as louer un bateau pour', {255,0,255}, '$' .. 30)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerServerEvent("fishing:lowmoney", 400) 
		TriggerEvent("chatMessage", 'Tu as louer un bateau pour', {255,0,255}, '$' .. 40)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat5' then
		...
		TriggerEvent("chatMessage", 'Tu as louer un bateau pour', {255,0,255}, '$' .. 45)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat6' then
		...
		TriggerEvent("chatMessage", 'Tu as louer un bateau pour', {255,0,255}, '$' .. 35)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'police' then
		...
		TriggerEvent("chatMessage", 'Vous avez pris un bateau')
		SetPedCoordsKeepVehicle(ped, x, y , z)
