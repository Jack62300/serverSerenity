	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Naviguez avec ~y~Z,Q,S,D~s~ et confirmez avec ~y~SPACE~s~ pour le bloc de code gauche.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Utilisez les touches ~y~Arrow Keys~s~ et ~y~ENTER~s~ pour le bloc de code droit")
			else
