...
local emscamera			  	  = false
local villecamera			  = false
...
AddEventHandler('esx_securitycam:hasEnteredMarker', function (zone)
  ...
  if zone == 'Cameras2' and not menuopen then
	CurrentAction     = 'camerasems'
	CurrentActionMsg  = _U('marker_hint')
  end
...
Citizen.CreateThread(function()
  while true do
	...
	for k,v in pairs(Config.Zones2) do    ---ems
	if PlayerData.job ~= nil and PlayerData.job.name ~= 'unemployed' and PlayerData.job.name == "ambulance" then
	  if(GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < 1.5) then
		isInMarker  = true
		currentZone = k
		...
		for a = 1, #Config.Locations do
			if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'cameras' then
					...
					if not menuopen then
						...
						{label = _U('ville'), value = 'villemenu'}
						}
				...
				elseif data.current.value == 'emsmenu' then
					menu.close()
					emscamera = true
					blockbuttons = true
						local pP = GetPlayerPed(-1)
						local firstCamx = Config.Locations[a].emsCamera[1].x
						local firstCamy = Config.Locations[a].emsCamera[1].y
						local firstCamz = Config.Locations[a].emsCamera[1].z
						local firstCamr = Config.Locations[a].emsCamera[1].r
						SetFocusArea(firstCamx, firstCamy, firstCamz, firstCamx, firstCamy, firstCamz)
						ChangeSecurityCamera(firstCamx, firstCamy, firstCamz, firstCamr)
						SendNUIMessage({
							type = "enablecam",
							label = Config.Locations[a].emsCamera[1].label,
							box = Config.Locations[a].emsLabel.label
						})
				...
				elseif data.current.value == 'villemenu' then
					menu.close()
					villecamera = true
					blockbuttons = true
						local pP = GetPlayerPed(-1)
						local firstCamx = Config.Locations[a].villecamera[1].x
						local firstCamy = Config.Locations[a].villecamera[1].y
						local firstCamz = Config.Locations[a].villecamera[1].z
						local firstCamr = Config.Locations[a].villecamera[1].r
						SetFocusArea(firstCamx, firstCamy, firstCamz, firstCamx, firstCamy, firstCamz)
						ChangeSecurityCamera(firstCamx, firstCamy, firstCamz, firstCamr)
						SendNUIMessage({
							type = "enablecam",
							label = Config.Locations[a].villecamera[1].label,
							box = Config.Locations[a].villeLabel.label
						})
			...
			if createdCamera ~= 0 then
				...
				if IsControlJustPressed(0, Keys["LEFT"]) then
					...
					elseif emscamera then 
					local newCamIndex

						if currentCameraIndexIndex == 1 then
							newCamIndex = #Config.Locations[currentCameraIndex].emsCamera
						else
							newCamIndex = currentCameraIndexIndex - 1
						end

						local newCamx = Config.Locations[currentCameraIndex].emsCamera[newCamIndex].x
						local newCamy = Config.Locations[currentCameraIndex].emsCamera[newCamIndex].y
						local newCamz = Config.Locations[currentCameraIndex].emsCamera[newCamIndex].z
						local newCamr = Config.Locations[currentCameraIndex].emsCamera[newCamIndex].r
						SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
						SendNUIMessage({
							type = "updatecam",
							label = Config.Locations[currentCameraIndex].emsCamera[newCamIndex].label
						})
						ChangeSecurityCamera(newCamx, newCamy, newCamz, newCamr)
						currentCameraIndexIndex = newCamIndex
					elseif villecamera then
						local newCamIndex

						if currentCameraIndexIndex == 1 then
							newCamIndex = #Config.Locations[currentCameraIndex].villecamera
						else
							newCamIndex = currentCameraIndexIndex - 1
						end

						local newCamx = Config.Locations[currentCameraIndex].villecamera[newCamIndex].x
						local newCamy = Config.Locations[currentCameraIndex].villecamera[newCamIndex].y
						local newCamz = Config.Locations[currentCameraIndex].villecamera[newCamIndex].z
						local newCamr = Config.Locations[currentCameraIndex].villecamera[newCamIndex].r
						SetFocusArea(newCamx, newCamy, newCamz, newCamx, newCamy, newCamz)
						SendNUIMessage({
							type = "updatecam",
							label = Config.Locations[currentCameraIndex].villecamera[newCamIndex].label
						})
						ChangeSecurityCamera(newCamx, newCamy, newCamz, newCamr)
						currentCameraIndexIndex = newCamIndex
					end
				...
				if IsControlJustPressed(0, Keys["RIGHT"]) then
					...
					elseif emscamera then
						...
						if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].emsCamera then
							newCamIndex = 1
						else
							newCamIndex = currentCameraIndexIndex + 1
						end
					...
					elseif villecamera then
						...
						if currentCameraIndexIndex == #Config.Locations[currentCameraIndex].villecamera then
							newCamIndex = 1
						else
							newCamIndex = currentCameraIndexIndex + 1
						end
				...
				elseif Config.Locations[currentCameraIndex].emsCamera[currentCameraIndexIndex].canRotate then
					local getCameraRot = GetCamRot(createdCamera, 2)
...
Citizen.CreateThread(function()
	while true do
		for a = 1, #Config.Locations do
			if IsControlJustReleased(0, Keys['E']) and CurrentAction == 'camerasems' then

					if not menuopen then
						menuopen = true
						CurrentAction = nil
						local elements = {
						{label = _U('emshosp'), value = 'emsmenu'}
