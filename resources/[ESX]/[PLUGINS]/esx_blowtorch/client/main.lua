...
AddEventHandler('esx_blowtorch:clearweld', function(x,y,z)
		ESX.ShowNotification('Braquage banque principale, la police arrive!')
		clearweld = true
		Citizen.CreateThread(function()
			while clearweld do
				Wait(1000)
...
AddEventHandler('esx_blowtorch:stopblowtorching', function()
	...
	ESX.ShowNotification('Annuler le chalumeau')
end)
	...
	Citizen.CreateThread(function()
			...
			Citizen.CreateThread(function()
				while blowtorching do
						...
						ESX.ShowNotification('Animation en cour')
						blowtorchingtime = blowtorchingtime - 1
						if blowtorchingtime <= 0 then
							blowtorching = false
							ClearPedTasksImmediately(PlayerPedId())
							ESX.ShowNotification('quitter l\'animation')
						end
				end
			end)
