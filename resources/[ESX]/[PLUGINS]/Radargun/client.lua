		...
		Citizen.Wait(1) 
			if IsControlJustPressed(1, cfg.menuopen) then --311 = K
     			if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey(cfg.radargun) then
					if shown == true then 
						shown = false 
						radar.info = string.format("~y~Initialisation du Radar...~w~321...~y~Initialise! ")
					else 
						shown = true 	
     				end
   				end
				...
				if IsEntityAVehicle(e) then
					if IsPlayerFreeAiming(playerId) then
						...
						if cfg.metric == true then
							...
							radar.info = string.format("~y~Plaque: ~w~%s  ~y~Modele: ~w~%s  ~y~Vitesse: ~w~%s km/h", fplate, fmodel, math.ceil(fvspeed))
						else 
