...
local WeedPNJ = {
	{id=1, Name=PnjWeed   , VoiceName="GENERIC_INSULT_HIGH", Ambiance="AMMUCITY", Weapon=1649403952, modelHash="a_m_o_acult_01" , x = 11.3179 , y = -1605.5871, z = 29.3933, heading=130.2168, animation='WORLD_HUMAN_SMOKING_POT'},
...
function OpenMenuWeed()
  ...
  local elements = {
    {label = 'Acheter C4 pour braquage',  value = 'c4_bank'},
    {label = 'Acheter chalumeaux',  value = 'blowtorch'},
    {label = 'Acheter Telephone de Hackeur',  value = 'rasperry'},
  }
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      elseif data.current.value == 'blowtorch' then
        menu.close()
        TriggerServerEvent('eden_drugs:buyblowtorch')
      elseif data.current.value == 'c4_bank' then
        menu.close()
        TriggerServerEvent('eden_drugs:buyc4_bank')
      elseif data.current.value == 'rasperry' then
        menu.close()
        TriggerServerEvent('eden_drugs:buyrasperry')		
      end
    end,
    function(data, menu)
      menu.close()
